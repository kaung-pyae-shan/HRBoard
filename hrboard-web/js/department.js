export function fillDepartmentTable() {
   ...
   function fetchDepartments(searchKeyword) {
      const url = new URL("http://localhost:8080/api/department/count");
   ...
   function submitDepartmentForm(action) {
      ...
      const url =
         action === "add"
            ...
            : `http://localhost:8080/api/department/update/${departmentEditId}`;
...
export function listenForDepartmentDeleteEvent() {
   ...
   deleteButton.onclick = () => {
      fetch(`http://localhost:8080/api/department/delete/${departmentId}`, {
         method: "DELETE",
      })
         .then((response) => {
