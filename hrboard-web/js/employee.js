export function fillEmployeeTable() {
   ...
   function fetchEmployees(searchParams, page = 0, size = 10) {
      const url = new URL("http://localhost:8080/api/employee/search");
   ...
   function submitEmployeeForm(action) {
      ...
      const url =
         action === "add"
            ...
            : `http://localhost:8080/api/employee/update/${employeeEditId}`;
...
function loadEmployeeEditFormData(employeeId) {
   ...
   fetch(`http://localhost:8080/api/employee/${employeeId}`)
      ...
      .then((employee) => {
         ...
         selectOptionById(departmentSelect, employee.department);
         selectOptionById(positionSelect, employee.position);
      });
...
function loadDepartmentAndPositionField() {
   ...
   // Department Fetch
   const departmentFetch = fetch("http://localhost:8080/api/department")
      .then((response) => response.json())
      .then((departments) => {
         // Placeholder option
   ...
   // Position fetch
   const positionFetch = fetch("http://localhost:8080/api/position")
      .then((response) => response.json())
      .then((positions) => {
         // Placeholder option
         const placeholderOption = document.createElement("option");
